...
/**
 * Minimal event types compatible with Radix's PointerDownOutsideEvent / FocusOutsideEvent shape.
 */
...
export type InteractionGuardRootProps = React.ComponentPropsWithoutRef<
...
> & {
  ...
  /**
   * Selector to determine "inside" surfaces (e.g., [data-menu-surface])
   */
...
function isInsideScoped(
  ...
  if (
    surface &&
    (surface as Element).closest?.(`[${scopeAttr}="${scopeId}"]`)
  ) {
    return true
  }
...
const Root = React.forwardRef<HTMLDivElement, InteractionGuardRootProps>(
...
export type InteractionGuardBranchProps = React.ComponentPropsWithoutRef<
  typeof Primitive.div
> & {
  asChild?: boolean
  children?: React.ReactNode
