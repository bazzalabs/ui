import { textToId } from '../primitives/menu-model.js'
import type {
...
function injectCompletedLoaderResultsIntoSubmenu(
  ...
  if (result && result.data !== undefined && isDeepSearchEnabled) {
    console.log('💉 [injectCompletedLoaderResults] Injecting result:', {
      submenuId: id,
      pathKey,
      dataCount: result.data?.length,
      hasOriginalNodes: !!submenu.nodes,
      originalNodeCount: submenu.nodes?.length,
      hasOriginalLoader: !!submenu.loader,
    })
