// @bazza-ui/data-view — Column Factory
...
export function createColumn<TData>(
  ...
  // No-op placeholder for state helpers — the hook attaches real closures.
  const noop = () => {}
  const noopFalse = () => false as const
  const noopUndefined = () => undefined
  const noopMinusOne = () => -1
  ...
  const column: Column<TData> = {
    ...
    // State helpers — overwritten by the hook with real closures
    getIsFiltered: noopFalse,
    getFilterValue: noopUndefined,
    getBaseFilterValue: noopUndefined,
    getOverrideFilterValue: noopUndefined,
    getIsSorted: noopFalse,
    getSortIndex: noopMinusOne,
    setFilterValue: noop,
    addFilterValue: noop,
