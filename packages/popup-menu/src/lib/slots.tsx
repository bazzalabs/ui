 ...
 */
export function defaultSlots<T>(): Required<PopupMenuSlots<T>> {
  return {
    ...
    Empty: ({ query }) => (
      <div data-slot="menu-empty">
        No results{query ? ` for "${query}"` : ''}.
      </div>
    ),
    Loading: ({ isFetching, progress, query, loadMode }) => (
      <div data-slot="menu-loading" data-load-mode={loadMode}>
    ...
    Item: ({ node, bind }) => {
      ...
      const isChecked = (props as any)['data-checked'] ?? false
