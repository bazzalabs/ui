...
- `Filter.Subject` - Filter subject/column name display
- `Filter.Operator` - Filter operator display/editor
- `Filter.Value` - Filter value display/editor
...
<Filter.Provider value={{ columns, filters, actions, strategy }}>
  <Filter.Root className="flex-col gap-4">
    <Filter.Block className="bg-muted h-8">
      <Filter.Subject className="text-primary" />
      <Separator orientation="vertical" />
      <Filter.Operator className="text-sm" />
      <Separator orientation="vertical" />
      <Filter.Value />
      <Separator orientation="vertical" />
...
- **Granular filter blocks**: Full control over individual filter block composition with `Filter.Block`, `Filter.Subject`, `Filter.Operator`, and `Filter.Value`
...
- `DataTableFilterRootProps` → `FilterRootProps`

##### 3. Internal Components Renamed

All `_v2` suffixes have been removed from internal components:

- `FilterMenu_v2` → `FilterMenu`
- `OptionItem_v2` → `OptionItem`
- `TextItem_v2` → `TextItem`
...
1. Review the [examples](#examples) section above
2. Check the [Component](#component) section for usage patterns
3. Open an issue on [GitHub](https://github.com/kianbazza/ui/issues)
