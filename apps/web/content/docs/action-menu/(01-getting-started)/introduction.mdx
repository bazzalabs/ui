...
The library ships three kinds (`kind`) of nodes — `item`, `group`, and `submenu`.
...
{
  kind: 'item',
  variant: 'checkbox', // [!code highlight]
  id: 'done',
  label: 'Done',
  icon: <Status.Done />,
  checked: checked, // [!code highlight]
  onCheckedChange: setChecked, // [!code highlight]
}
```
...

{
  ...
  variant: 'radio', // [!code highlight]
  id: 'fruits',
  value: fruit, // [!code highlight]
  onValueChange: setFruit, // [!code highlight]
}
```
...
import { ReactQueryLoaderAdapter, createLoader } from '@bazza-ui/action-menu/react-query'
