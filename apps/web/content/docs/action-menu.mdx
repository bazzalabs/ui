...
This component uses a data-first API. You define your menu as a `MenuData` object and pass it to the root `ActionMenu.Surface` component:
...
Action menus are defined as plain data.

Each node in your MenuData describes what to render and how it should behave.

The library ships three first-class node kinds — item, group, and submenu.

### Item

An action that the user can run.

<TypeTable pkg="@bazza-ui/action-menu" type="ItemNode" />

### Group

A group of actions that the user can run.
