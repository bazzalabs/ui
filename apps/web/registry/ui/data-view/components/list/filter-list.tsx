...
import type { Column, FilterModel } from '@bazza-ui/data-view/react'
import {
...
const FilterList = forwardRef<HTMLDivElement, FilterListProps>(
  ({ className, children, ...props }, ref) => {
    const { filters, columns } = useDataViewContext()
    ...
    return (
      ...
      >
        {filters.map((filter) => {
          ...
          // Skip if no filter value or column not found
          if (!filter.values || !column) return null
