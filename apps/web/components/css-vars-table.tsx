...
interface CssVarsTableProps {
  /**
   * The enum name to display (e.g., "DropdownMenuPositionerCssVars")
   */
  type: string
  /**
...
/**
 * Renders a table of CSS custom properties from an enum in the generated metadata
 */
export function CssVarsTable({ type, pkg }: CssVarsTableProps) {
  ...
  return (
    <div className="my-6 border border-border rounded-md overflow-hidden">
      <div className="w-full text-sm">
        {/* Header */}
        <div className="grid grid-cols-[35%_20%_45%] bg-neutral-100 dark:bg-neutral-900 border-b border-border">
          <div className="px-4 py-3 font-mono font-semibold">Variable</div>
        ...
        <div className="bg-white dark:bg-black">
          {enumMeta.members.map((member) => (
            <div
              key={member.name}
              className={cn(
                'grid grid-cols-[35%_20%_45%] border-b border-border last:border-b-0',
                'hover:bg-muted/50 transition-colors',
              )}
            >
              <div className="px-4 py-2 flex items-start">
                <code className="rounded-sm bg-violet-300/25 dark:text-violet-300 text-violet-700 dark:bg-violet-600/40 px-[0.35rem] py-[0.2rem] text-sm font-mono">
                  {member.value}
                </code>
              </div>
              <div className="px-4 py-2">
                {member.valueType ? (
                  <code className="rounded-sm bg-muted px-[0.35rem] py-[0.2rem] text-sm font-mono text-muted-foreground">
                    {member.valueType.replace(/^\{|\}$/g, '')}
                  </code>
                ) : (
